...
REGISTER_RESPONSE=$(curl -s -X POST http://localhost:3000/api/auth/register \
  ...
  -d '{"email":"demo3@abetworks.com", "password":"DemoPass123!", "firstName":"Demo", "lastName":"User3", "tenantId":"94e0e88b-37d8-4e72-9443-2ce2b20328e7", "role":"client_user"}')
...
CUSTOMER_RESPONSE=$(curl -s -X POST http://localhost:3000/api/customers \
  ...
  -d '{"first_name":"John", "last_name":"Doe", "email":"john.doe.demo@example.com", "company":"ABC Corp", "phone":"+1234567890", "status":"customer"}')

if [[ $CUSTOMER_RESPONSE == *"id"* && $CUSTOMER_RESPONSE == *"john.doe.demo@example.com"* ]]; then
...
LEAD_RESPONSE=$(curl -s -X POST http://localhost:3000/api/leads \
  ...
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "x-tenant-id: abetworks" \
  -d '{"first_name":"Jane", "last_name":"Smith", "email":"jane.smith.demo@example.com", "company":"XYZ Inc", "status":"new", "value":7500}')
...
DEAL_RESPONSE=$(curl -s -X POST http://localhost:3000/api/deals \
  ...
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "x-tenant-id: abetworks" \
  -d '{"title":"New Project", "description":"Sample deal description", "value":15000, "status":"open", "probability":25}')

if [[ $DEAL_RESPONSE == *"id"* && $DEAL_RESPONSE == *"New Project"* ]]; then
...
TASK_RESPONSE=$(curl -s -X POST http://localhost:3000/api/tasks \
  ...
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "x-tenant-id: abetworks" \
  -d '{"title":"Follow up with client", "description":"Need to call the client to discuss project", "priority":"high", "due_date":"2026-12-31"}')

if [[ $TASK_RESPONSE == *"id"* && $TASK_RESPONSE == *"Follow up with client"* ]]; then
...
CUSTOMERS_COUNT=$(curl -s -X GET "http://localhost:3000/api/customers?limit=100" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "x-tenant-id: abetworks" | grep -o '"data":' | wc -l)

LEADS_COUNT=$(curl -s -X GET "http://localhost:3000/api/leads?limit=100" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "x-tenant-id: abetworks" | grep -o '"data":' | wc -l)

echo "📊 Retrieved data - Customers: $CUSTOMERS_COUNT, Leads: $LEADS_COUNT"
echo ""
